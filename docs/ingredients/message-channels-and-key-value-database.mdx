...
Facilitating communication between behaviors using custom messages and synced values to synchronize state or trigger
actions.
...
export default class GlobalStats extends Behavior {
  ...
  private playerClicks = new Map<string, { nickname: string; clicks: number }>();
  ...
  private async persistPlayer(playerId: string, nickname: string) {
    ...
    const updatedPlayers = [
      ...allPlayers.filter((player) => typeof player === "object" && player.playerId !== playerId),
      { playerId, nickname },
    ];
