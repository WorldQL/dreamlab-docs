...
Dreamlab runs at a fixed, customizable tickrate. By default, the engine will tick 60 times a second. During a tick, the engine will update entities and run the `onTick` functions in your [Behavior scripts](Behaviors.mdx).
...
`onPostTick` is always guaranteed to run after `onTick`. It's useful to think of your tick methods as *preparing the next world state*. `onTick` is the first step of preparing the next state and `onPostTick` is the second step.
