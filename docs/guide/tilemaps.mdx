...
1. Right click on the _Scene Graph_ and click **New Entity → Tilemap**.
...
| `bounds` _(getter)_                   | `{ width, height, offset }` in tile units.                                     |

### `TileInfo` union

```ts
type TileInfo = { type: "atlas"; id: number } | { type: "color"; color: number };
```
...
Set **Scale filter mode** to "nearest" on the Tilemap _or_ Camera.
