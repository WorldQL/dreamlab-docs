...
as the `onTick()` method which gets called every engine tick (60 TPS by default). See [Tick Loop](./tick-loop.mdx) for
more info on engine ticks.

All behaviors also have access to some global properties (such as `.entity`) which let them interact with the game
state. You can also define your own properties as [synced values](./synced-values.mdx) to automatically sync them over
the network, as well as make them available in the Dreamlab Editor.

:::info

Marking a behavior property as a synced value is similar to the `[SerializeField]` annotation in Unity or the `@export`
annotation in Godot, with the added benefit of enabling automatic network syncing.
...
Due to a limitation with TypeScript using the platform-specific lifecycle methods will **not** narrow the type on
`this.game` to `ClientGame` or `ServerGame`.
