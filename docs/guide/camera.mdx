...
<img src={require("../../static/img/image-3.png").default} alt="Camera bounds preview" width="250" />
The **Camera** entity controls what the player sees. A white box shows its “guaranteed-visible” area—everything inside
that box is on-screen no matter the monitor's aspect ratio.
...
> **Tip:** setting `active = true` on a camera automatically sets `active = false` on any other cameras, so you never
> have two active at once.
